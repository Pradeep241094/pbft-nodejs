...
class P2pserver {
  ...
  broadcastTransaction(transaction) {
    console.log('>>>>>>>>>>time stamp while broadcast', new Date());
  ...
  broadcastPrePrepare(block) {
    // console.log('>>>>>>>>Broadcast prepare @ time', new Date())
    this.sockets.forEach(socket => {
  ...
  sendPrePrepare(socket, block) {
    // console.log("Send Pre prepare @ timestamp", new Date())
    socket.send(
      JSON.stringify({
  ...
  sendPrepare(socket, prepare) {
    const sendPrepare = process.hrtime.bigint();
    console.log('>>>send prepare time', sendPrepare)
    socket.send(
      JSON.stringify({
  ...
  sendCommit(socket, commit) {
    const atCommitStage = process.hrtime.bigint();
    console.log('>>>atCommitStage', atCommitStage)
    socket.send(
      JSON.stringify({
  ...
  sendRoundChange(socket, message) {
    const roundChange = process.hrtime.bigint();
    console.log('>>>roundChange time', roundChange)
    socket.send(
      JSON.stringify({
  ...
  messageHandler(socket) {
    ...
    socket.on("message", message => {
      ...
      console.log("TRANSCATION MSG TYPE", data.type)
      // select a perticular message handler
      switch (data.type) {
        case MESSAGE_TYPE.transaction:
          const transactionTime = process.hrtime.bigint();
          console.log('>>>Transaction Stage', transactionTime)
          ...
          if (
            ...
            if (thresholdReached) {
              ...
              if (this.blockchain.getProposer() == this.wallet.getPublicKey()) {
                ...
                const sendTransactionTime = process.hrtime.bigint();
                console.log('>>>sendTransactionTime', sendTransactionTime)
        ...
        case MESSAGE_TYPE.pre_prepare:
          ...
          if (
            ...
            const Prepreprare = process.hrtime.bigint();
          console.log('>>>Preprepare time', Prepreprare)
            // add block to pool
        ...
        case MESSAGE_TYPE.prepare:
          const prepareiime = process.hrtime.bigint();
        ...
        case MESSAGE_TYPE.commit:
          const commitTime = process.hrtime.bigint();
          ...
          if (
            ...
            // add to pool
            const commitAddedTime = process.hrtime.bigint();
            console.log('>>>commit Added', commitAddedTime)
            ...
            if (
              ...
              const updatedBlockTime = process.hrtime.bigint();
              console.log('>>>Add updated Block', updatedBlockTime)
            }
